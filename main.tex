...
Let $S$ be a set. A group $F(S)$ equipped with a map $\iota:S\to F(S)$ is a free group generated by $S$ if for any group $H$ and any map $f:S\to H$, there exists a unique homomorphism $\Psi:F(S)\to H$ such that $\Psi\circ\iota=f$.
\end{definition}
\begin{theorem}
Let $S$ be a set and let $F(S)$ be the group consisting of reduced words in $S$, along with the usual inclusion map $\iota:S\to F(S)$. Then $F(S)$ is a free  group generated by $S$.
\end{theorem}
\begin{proof}
Let $f:S\to H$ be a map to a group $H$. There then exists a unique homomorphism $\Psi:F(S)\to H$ such that $\Psi\circ\iota=f$, namely the one given by $\Psi(s_1^{\epsilon_1}s_2^{\epsilon_2}\cdots s_m^{\epsilon_m})=\Psi(\iota(s_1)^{\epsilon_1}\iota(s_2)^{\epsilon_2}\cdots\iota(s_m)^{\epsilon_m})=f(s_1)^{\epsilon_1}f(s_2)^{\epsilon_2}\cdots f(s_m)^{\epsilon_m}$.
\end{proof}
\begin{theorem}
Let $S$ be a set and let $F(S)$, equipped with a map $\iota:S\to F(S)$, and $F(S)'$, equipped with a map $\iota':S\to F(S)'$, be free groups generated by $S$. Then there exists a unique isomorphism $h:F(S)\to F(S)'$ such that $\iota'=h\circ\iota$.
\end{theorem}
\begin{proof}
By the universal property of free groups, there exists a unique homomorphism $\Psi:F(S)\to F(S)'$ such that $\Psi\circ\iota=\iota'$, and similarly, there exists a unique homomorphism $\Psi':F(S)'\to F(S)$ such that $\Psi'\circ\iota'=\iota$. Hence, $\iota'=(\Psi\circ\Psi')\circ\iota'$, and so, by uniqueness, $\Psi\circ\Psi'=\text{id}_{F(S)'}$. Similarly, $\Psi'\circ\Psi=\text{id}_{F(S)}$. Hence, $\Psi$ is an isomorphism.
\end{proof}
\begin{theorem}[Every group has a presentation]
Let $G$ be a group. Then there exists a free group $F$ and a normal subgroup $N\trianglelefteq F$ such that $G\cong F/N$.
\end{theorem}
\begin{proof}
Let $S$ be a generating set of $G$ and let $F$ be the free group generated by $S$, with $\iota:S\to F$ being the inclusion map. Let $f:S\to G$ be an inclusion map on sets. Then by the universal property of free groups, there exists a unique homomorphism $\Psi:F\to G$ such that $\Psi\circ\iota=f$. This in particular implies that $S\subseteq\text{Im }\Psi$, and hence that $\Psi$ is surjective. The first isomorphism theorem then gives $G\cong F/\ker\Psi$, as required.
\end{proof}
